...
export function HotelDetail() {
  ...
  const handleDateChange = (range) => {
    if (range[0] && isDateInPast(range[0])) {
      setAlertMessage("You cannot select a date in the past.");
      return;
    }
    if (range[1] && isRangeInvalid(range)) {
      setAlertMessage("You cannot select a range that includes booked dates.");
      return;
    }
  ...
  const PropertyCard = () => (
    <div className="border border-gray-300 rounded-lg p-6 shadow-md w-full">
      ...
      <div className="flex justify-between text-sm text-gray-600 mb-4">
        ...
        <div className="flex items-center space-x-1">
          <span>💰</span>
          <span>${price.toFixed(2)} per night</span>
        </div>
      </div>
      ...
      >
        ...
        {remainingAmenities > 0 && (
          <Button variant="light" color="blue" size="xs">
            View more {remainingAmenities} amenities
          </Button>
        )}
      </Card>
    );
      ...
      >
        ...
        <Table>
          <tbody>
            ...
            <tr>
              <td className="py-2">Friday - Sunday</td>
              <td className="py-2 text-right">${weekendRate.toFixed(2)}</td>
            </tr>
            <tr>
              <td className="py-2">Rent by month</td>
            ...
            <tr>
              <td className="py-2">Minimum number of nights</td>
              <td className="py-2 text-right">1 night</td>
            </tr>
            <tr>
              <td className="py-2">Max number of nights</td>
              <td className="py-2 text-right">90 nights</td>
            </tr>
          </tbody>
        </Table>
      </Card>
  ...
  const Availability = () => (
    ...
    >
      ...
      <Text size="sm" color="dimmed" className="mt-4">
        Note: The dates that are disabled have already been booked.
      </Text>
    </Card>
  );
  ...
  const HostInformation = () => {
    ...
    if (isPending) return <div>Loading host information...</div>;
    if (isError)
      return <div>Error loading host information: {error.message}</div>;
  ...
  const Reviews = () => {
    if (isReviewsPending) return <div>Loading reviews...</div>;
    if (isReviewsError)
